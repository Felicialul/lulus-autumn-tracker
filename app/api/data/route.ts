...
import { applications, attachments, interviews, knowledgeNotes, offers, prospects, resumes, timelineEvents, userSettings } from "../../../db/schema";
...
export async function GET() {
  try {
    const db = getDb();
    const [applicationRows, interviewRows, timelineRows, noteRows, prospectRows, resumeRows, offerRows, attachmentRows, settingsRows] = await Promise.all([
      ...
      db.select().from(resumes).orderBy(desc(resumes.createdAt), desc(resumes.id)),
    ...
    return Response.json({ applications: applicationRows, interviews: interviewRows, timeline: timelineRows, notes: noteRows, prospects: prospectRows, resumes: resumeRows, offers: offerRows, attachments: attachmentRows, settings: settingsRows[0] ?? null });
  } catch (error) {
